/**
 * useGeminiLive — WebSocket hook for Gemini Live API communication.
 * Handles connection lifecycle, audio/video streaming, and server responses.
 */
...
export function useGeminiLive(
  ...
  const sendVideoFrame = useCallback((jpegData: ArrayBuffer) => {
    ...
    wsRef.current.send(JSON.stringify({
      type: "video_frame",
      data: arrayBufferToBase64(jpegData),
    }));
  }, []);
